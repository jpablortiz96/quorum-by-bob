...
function ADRModal({ adr, onClose }: { adr: ADR; onClose: () => void }) {
  return (
    ...
    >
      ...
      <motion.div
        className="relative bg-[#141414] border border-[#262626] rounded-xl max-w-3xl w-full max-h-[80vh] flex flex-col z-10"
        initial={{ scale: 0.95, y: 16 }}
        animate={{ scale: 1, y: 0 }}
      >
        <div className="flex items-center justify-between p-6 border-b border-[#262626]">
          <div>
            <h3 className="text-lg font-semibold text-[#fafafa]">{adr.id}</h3>
            <p className="text-sm text-[#a3a3a3]">{adr.title}</p>
          </div>
          <div className="flex items-center gap-3">
            <a
              href={`/api/download/${adr.id}`}
              download
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-[#d4af3720] text-[#d4af37] hover:bg-[#d4af3730] transition-colors"
            >
            ...
            <button
              onClick={onClose}
              className="text-[#737373] hover:text-[#fafafa] transition-colors px-2"
            >
              ✕
            </button>
          </div>
        ...
        <div className="overflow-y-auto p-6">
          <pre className="text-xs text-[#a3a3a3] font-mono whitespace-pre-wrap leading-relaxed">
            {adr.content}
          </pre>
        </div>
      </motion.div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left border-b border-[#1f1f1f]">
              {["ID", "Title", "Status", "DCS", "Date", "Actions"].map((h) => (
                <th key={h} className="pb-3 pr-4 text-xs font-medium uppercase tracking-wider text-[#737373]">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-[#1f1f1f]">
            {adrs.map((adr) => (
              <tr key={adr.id} className="group hover:bg-[#141414] transition-colors">
                <td className="py-3 pr-4">
                  <span className="font-mono text-xs text-[#d4af37]">{adr.id}</span>
                </td>
                <td className="py-3 pr-4 max-w-xs">
                  <span className="text-[#fafafa] text-sm">{adr.title}</span>
                </td>
                <td className="py-3 pr-4">
                ...
                <td className="py-3 pr-4 text-[#737373] text-xs font-mono whitespace-nowrap">
                  {adr.date}
                </td>
                <td className="py-3">
                  <button
                    onClick={() => setSelectedADR(adr)}
                    className="flex items-center gap-1 text-xs text-[#a3a3a3] hover:text-[#fafafa] transition-colors"
                  >
